...
def name_of(obj) -> str | None:
    """What an object is called, read off its ``meta`` component.

    Since contract v6 that is the only way there is: the engine's ``NameComponentData`` went with
    every other engine-declared record, and identity, name and the parent link travel together in
    one payload the FORMAT owns.
    """
    payload = payload_for(obj, well_known.META_ID)
    return None if payload is None else payload.get(well_known.NAME)
...
def write_authoring_schema() -> None:
    ...
    From a checked-in FIXTURE, because since contract v6 there is nothing to generate it from: the
    engine declares no authored components at all, and the bridge verb that printed them was
    removed with the constant it read. See tests/fixtures/README.md.
    """
...
def main() -> int:
    ...
    print()
    check(document["SchemaVersion"] == 6, "schema version is 6")
    ...
    # -- axis conversion --------------------------------------------------------------
    #
    # Read off the decomposed TRS rather than out of a matrix. v5 wrote one flattened world matrix
    # and these assertions dug the translation out of indices 12/13/14 and recovered the scale as
    # column lengths; v6 writes local position, rotation and scale as three arrays, so the values
    # are simply there. Both objects here are roots, so local IS world and the numbers are the
    # same ones this test always checked.
    ball = objects["Ball"]
    position = payload_for(ball, well_known.TRANSFORM_ID)[well_known.POSITION]
    check(approx(position, [1.0, 3.0, -2.0]),
          "Blender (1, 2, 3) converts to contract (1, 3, -2)", str(position))

    ground = objects["Ground"]
    scale = payload_for(ground, well_known.TRANSFORM_ID)[well_known.SCALE]
    check(approx(scale, [20.0, 0.5, 20.0]),
          "Blender scale (20, 20, 0.5) converts to contract (20, 0.5, 20)", str(scale))
